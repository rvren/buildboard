...
function insertNodeTree(
  ...
  db.prepare(
    `INSERT INTO nodes(id, project_id, owner_kind, owner_id, parent_id, order_index,
       type, name, props, styles, bindings, action, repeat, instance_of, overrides, responsive, variant, hidden, visible_if, locked)
...
const saveTx = (db: Database.Database, p: Project) => {
  ...
  p.screens.forEach((s, i) => {
    db.prepare(
      `INSERT INTO screens(id, project_id, name, title, description, path, background, width, height, x, y, data_source_id, order_index)
       VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    ).run(
      s.id, p.id, s.name, s.title ?? null, s.description ?? null, s.path ?? null, s.background ?? null,
...
export function getProject(db: Database.Database, id: string): Project | null {
  ...
  const screens: Screen[] = (
    ...
    if (s.background != null) screen.background = s.background;
